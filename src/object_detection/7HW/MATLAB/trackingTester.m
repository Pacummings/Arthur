...
%     track_hist = countcats(cata_sample(:));
    ...
    for i=data_params.frame_ids(2:end)
        ...
        search_rangey = uint8(max((cy-ss),1):(min((cy+ss),size(ith_frame,1))-1));
        search_rangex = uint8(max((cx-ss),1):(min((cx+ss),size(ith_frame,2))-1));
        search_space = ith_frame(  search_rangey, search_rangex, :);
        indexed_search_space = rgb2ind(search_space, track_map);
        candidate_columns = im2col(indexed_search_space, [rec_h, rec_w]);
...
%         candidate_errors = candidate_s - repmat(track_hist, [1 size(candidate_hists,2)]);
...
%         [~, minindx] = min(square_sum_errors);
        ...
        result = drawBox(ith_frame, [atlx, atly, rec_w, rec_h], [255,255,255], 3);
